...
class AbstractWebsite_ByClass(AbstractWebsite):
    def __init__(self):
        super().__init__()

    # 透過classname針對一個頁籤爬蟲
    @classmethod
    def reptile_by_class(cls, name, attributes, url, target="textContent"):
        # 1. 建立網頁驅動
        ...
        # 4. 計算網頁排名陣列
        web_rank_string_array = common_use.calculateWebRankDataByClassNameElements(attributes,
                                                                                   wd,
                                                                                   target)
